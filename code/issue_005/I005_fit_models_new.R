...
I005_fit_models_new = function(data,
  ...
  for (model_count in c(3,4)) {
    ...
    # Check if different l parameters influence model fit considering different updates
    # E.g.: if l is a meaningful parameter it should have a strong impact on LL when predicting choices with very low PEs
    pe_width = 5
    for(pe_band_high in seq(pe_width,70, by = pe_width)){
      
      # Get full interval of PEs
      pe_band_low = pe_band_high - pe_width
      ...
      for(new_lower_count in seq(length(lower_grid))){
        ...
        # If reward was experienced at trial i, the respective PE is entered at i+1, together with the updated value (that puts the previous experienced PE in the same line as the choice we want to look at)
        # Add previously experience PE to data (also considering forced choices)
        i_05_pe = cres[[2]] %>%
          # Restrict trials to 1v2 comparisons
          #.[bandit_index, ] %>%
          # Add LAST EXPERIENCED PE for a certain choice/trial
          .[, ':='(updates_b1 = cres[[3]][1,trial],
                   updates_b2 = cres[[3]][2,trial])]
        
        # Index for choices that come after a specific PE interval (dependent on bandit_index)
        # Q: Do we need to consider choices after a PE even if the bandit was NOT chosen?
        pe_index = which(
          (abs(i_05_pe$updates_b1) > pe_band_low & abs(i_05_pe$updates_b1) <= pe_band_high) |
            (abs(i_05_pe$updates_b2) > pe_band_low & abs(i_05_pe$updates_b2) <= pe_band_high)
        )
        # Intersection of bandit 1v2 trials, and trials in which previous PE was in specified band
        cidx = bandit_index[bandit_index %in% pe_index]
        
        # If there are no choices to consider, abort
        if(length(cidx) == 0){
          break
        }
        #-----
        ...
        temp$l = new_lower
        temp$pe_band_low = pe_band_low
        temp$pe_band_high = pe_band_high
        # sort columns
        ...
        temp_pes$l_bound_iteration = new_lower_count
        temp_pes$l = new_lower
        temp$pe_band_low = pe_band_low
        temp$pe_band_high = pe_band_high
        # Fuse PE output
