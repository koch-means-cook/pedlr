...
Fit_Pedlr_simple_const = function(data,
                            params.alpha1,
                            params.temperature,
  ...
  for(trial_count in 1:params.ntrials){
    ...
    # Normal trials (no time out): Choice and updating of model
      ...
      # Otherwise update
        ...
        # Simple pedlr model with constant alpha0 component
        fpe = 0.1 + (0.9 * (params.alpha1 * (abs(pe)/max_pe_so_far)))
        updated_value = choice_value + fpe * pe
      }
