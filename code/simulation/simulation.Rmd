...
source(file.path(source_path, 'Gaussian_pseudo_sim.R',
                 fsep = .Platform$file.sep))
source(file.path(source_path, 'Uniform_pseudo_sim.R',
                 fsep = .Platform$file.sep))
source(file.path(source_path, 'Bimodal_pseudo_sim.R',
                 fsep = .Platform$file.sep))
source(file.path(source_path, 'Create_design_complete.R',
                 fsep = .Platform$file.sep))
source(file.path(source_path, 'Sample_subjects.R', fsep = .Platform$file.sep))
source(file.path(source_path, 'PE_dep_LR_choice_model.R',
                 fsep = .Platform$file.sep))
...
source(file.path(source_path, 'PE_dep_LR_single_model.R',
                 fsep = .Platform$file.sep))
...
source(file.path(source_path, 'Model_perc_correct.R',
                 fsep = .Platform$file.sep))
source(file.path(source_path, 'Model_bias_correct.R',
                 fsep = .Platform$file.sep))
```
...
# Parameters to save and load data output
file = file.path(derivatives_path, 'bias_correct_simulation_pedlr.tsv')
save_data = FALSE
save_file = file
load_data = TRUE
load_file = file
...
# Parameters to save and load data output
file = file.path(derivatives_path, 'bias_correct_simulation_pedlr_interdep.tsv')
save_data = FALSE
save_file = file
load_data = TRUE
load_file = file
...
```{r, fig.width=6, fig.height=6}
#data_plot = subset(data_bias_correct, task_version == 1)
# data_plot = data_bias_correct
...
# 
# p_v1 = ggplot(data_plot, aes(x = comp,
#                              y = perc_correct,
...
#   facet_grid(temperature + alpha0 ~ task_version + alpha1) +
#   theme(legend.position = 'right')
# 
# p_v1

# data_plot = subset(data_bias_correct, task_version == 2)
# data_plot = ddply(data_plot,
#                   .(task_version, comp, distance_shrinkage, alpha0, alpha1, temperature, choice_policy),
#                   summarize,
#                   perc_correct = mean(perc_correct))
# 
# p_v2 = ggplot(data_plot, aes(x = comp,
#                              y = perc_correct,
#                              group = distance_shrinkage,
#                              color = distance_shrinkage)) +
#   geom_line() + 
#   scale_color_viridis(option='D') +
#   scale_y_continuous(limits=c(0.45,1)) +
#   facet_grid(alpha0 ~ task_version + temperature + alpha1) +
#   theme(legend.position = 'right')
# 
# plot_grid(p_v1, p_v2,
#           ncol = 1)
...
#                          dist_name = 'bimodal',
#                          reward_space_lb = 1,
#                          reward_space_ub = 100)$outcome
# sd(bla)
# hist(bla, breaks = seq(1,100,1))
```
