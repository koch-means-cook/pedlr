...
# Parameters for bias assessment
shrink_distance_vec = seq(0,100/6, length.out = 50)
```
...
# Parameters to save and load data output
file = file.path(derivatives_path, 'shrinkage_simulation_pedlr.tsv')
save_data = FALSE
save_file = file
load_data = TRUE
load_file = file
...
p = ggplot(data_plot, aes(x = distance_shrinkage, y = mean_balance)) +
  ...
  geom_vline(xintercept = 100/6, linetype='dashed', color='red') +
  facet_wrap(~task_version)
...
# Parameters to save and load data output
file = file.path(derivatives_path, 'shrinkage_simulation_pedlr_interdep.tsv')
save_data = TRUE
save_file = file
load_data = FALSE
load_file = file
