...
source(file.path(source_path, 'Model_bias_value.R', fsep = .Platform$file.sep))
source(file.path(source_path, 'Model_perc_correct.R', fsep = .Platform$file.sep))
source(file.path(source_path, 'Model_bias_correct.R', fsep = .Platform$file.sep))
```
...
```{r}
# See if bias assessment was already done
file = file.path(derivatives_path, 'bias_simulation.tsv')
# If yes, load output
if(file.exists(file)){
  data_bias = read.table(file, sep = '\t', header = TRUE)
  
  # Only run bias assessmentin case not present already
  ...
  write.table(data_bias, file, sep='\t', row.names = FALSE)
}
...
**Annotation:** This only holds in case the sampling means are hierarchical (reward_stim_1 < reward_stim_2 < reward_stim_3).

```{r}
perc_correct = Model_perc_correct(data_model)

data_choice = perc_correct$perc_correct
perc_correct$plot
```

### Influence of distribution distance on correct choices

**Annotation:** Here we will use a gain and a loss beta for simplicity.

```{r}
# Steps in which to loop over each parameter
steps_alpha0 = seq(from = 0, to = 1, length.out = 3)
steps_alpha1 = seq(from = 0, to = 1, length.out = 3)
steps_temperature = 1
...
perc_forced = 20
blocks_per_task = 3
dist_list = dist_list = list(c('gaussian', (100/6) * 1, gaussian_sd),
                 c('beta', 5/3, (2*5)/3),
                 c('gaussian', (100/6) * 3, gaussian_sd),
                 c('gaussian', (100/6) * 3, gaussian_sd),
                 c('beta', (2*5)/3, 5/3),
                 c('gaussian', (100/6) * 5, gaussian_sd))
model = 'Pedlr'
parameters = parameters
init_values = list(c(50,50,50),
                   c(50,50,50))
shrink_distance_vec = seq(3)
file = file.path(derivatives_path, 'bias_correct_simulation.tsv')
save_data = FALSE
save_file = file
load_data = FALSE
load_file = file

data_bias_correct = Model_bias_correct(n_subjects = n_subjects,
                                       set_seed = set_seed,
                                       n_blocks = n_blocks,
                                       perc_forced = perc_forced,
                                       blocks_per_task = blocks_per_task,
...
```{r}
data_plot = subset(data_bias_correct, alpha0 == 0.5)
data_plot = ddply(data_plot,
                  .(task_version, comp, distance_shrinkage, alpha0, alpha1, temperature, choice_policy),
                  summarize,
                  perc_correct = mean(perc_correct))

ggplot(data_plot, aes(x = comp,
                      y = perc_correct,
  ...
  facet_wrap(~task_version + alpha1)
```
