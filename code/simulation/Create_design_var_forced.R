...
Create_design_var_forced = function(n_miniblock,
                                    perc_forced,
                                    dist1_val1,
                                    dist1_val2,
                                    dist2_val1,
  ...
  for(block_count in c(1:n_miniblock)){
    mini_block = Create_miniblock_var_forced(block_count, perc_forced)
    design = rbind(design, mini_block)
  }
  
  # Sample rewards for design
  # Number of samples per mini_block (how often is a stimulus presented over each miniblock?)
  n_samples = c(mini_block$option_left, mini_block$option_right)
  n_samples = length(which(n_samples==1))
  n_samples = n_samples*n_miniblock
  ...
  # Stimulus 1
  # Left presentation
  left_index = which(design$option_left == 1)
  design$reward_stim_1[left_index] = rewards_1[1:length(left_index)]
  # Right presentation
  right_index = which(design$option_right == 1)
  design$reward_stim_2[right_index] = rewards_1[(length(left_index)+1):length(rewards_1)]
  
  # Stimulus 2
  left_index = which(design$option_left == 2)
  design$reward_stim_1[left_index] = rewards_2[1:length(left_index)]
  # Right presentation
  right_index = which(design$option_right == 2)
  design$reward_stim_2[right_index] = rewards_2[(length(left_index)+1):length(rewards_2)]
  
  # Stimulus 3
  left_index = which(design$option_left == 3)
  design$reward_stim_1[left_index] = rewards_3[1:length(left_index)]
  # Right presentation
  right_index = which(design$option_right == 3)
  design$reward_stim_2[right_index] = rewards_3[(length(left_index)+1):length(rewards_3)]
