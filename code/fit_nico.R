...
ids = as.vector(sapply(data_list, function(x) substr(x, 58, 64)))
nid = length(ids)
...
colnames(modeldf) = c('RW_LL', 'RW_alpha', 'RW_temp', 'RW_init_values', 'RW_value1', 'RW_value2', 'RW_value3', 'PEDLR_LL', 'PEDLR_alpha0', 'PEDLR_alpha1', 'PEDLR_temp', 'PEDLR_init_values',  'PEDLR_value1', 'PEDLR_value2', 'PEDLR_value3', 'Choice1v2', 'Choice2v3', 'Rating2', 'RatingSD2')

for (cid in 1:nid) {
  cat(paste('Fitting Sub ', sprintf("%02d", cid), ' ', sep = ''))
  data = Load_data() %>%
    Add_comp(.) %>%
    .[participant_id == ids[cid] & task_version == 1, ] %>%
  ...
  cat(paste(' | RW ', sprintf("%03d", round(modeldf$RW_LL[cid])), ' - PEDLR ', sprintf("%03d", round(modeldf$PEDLR_LL[cid])), sep = ''))
  cat(paste(' | LR RW ', sprintf("%01.2f", round(modeldf$RW_alpha[cid])),  ' - LR0 PEDLR ', sprintf("%01.2f", round(modeldf$LR_alpha0[cid])), ' - LR1 PEDLR ', sprintf("%01.2f", round(modeldf$LR_alpha1[cid])), sep = ''))
  cat(paste(' | Done\n', sep = '.'))
}
