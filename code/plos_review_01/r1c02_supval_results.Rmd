...
## Review Figure (avg. AICc & counts)

```{r}
# Take mean data for avg AICc
data_plot_mean = Prepare_data_for_plot(data_AICc)

# Plot without error-bars (other graph shows variability)
p_avg_aicc = ggplot(data = data_plot_mean,
                    aes(x = model,
                        y = AICc,
                        fill = model,
                        color = model)) +
  geom_point(aes(x = model,
             ...
             shape = 23,
             stroke = 1) +
  scale_y_continuous(limits = c(115,120)) +
  labs(y = 'Average AICc across groups') +
  scale_fill_manual(values = custom_guides)
p_avg_aicc = Neurocodify_plot(p_avg_aicc) +
  theme(panel.grid.major.x = element_blank(),
        panel.grid.minor.x = element_blank(),
        legend.position = 'none',
        axis.title.x = element_blank(),
...
# Combine plots for review
p = cowplot::plot_grid(p_avg_aicc, NULL, p_counts_AICc,nrow = 1, rel_widths = c(1,0.1,1),
                       align = 'hv', axis = 'tb')
p

# save to 7x4in
file = file.path(base_path, 'derivatives', 'plos_review_01', 'figures',
                 'r1c02', fsep = .Platform$file.sep)
ggsave(filename = paste0(file, '.pdf'), plot = p, device = 'pdf', width = 7,
       height = 4, units = 'in')
ggsave(filename = paste0(file, '.png'), plot = p, device = 'png',
