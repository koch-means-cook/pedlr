...
data_rw_beta = data_betas %>%
  .[model == 'rw',] %>%
  data.table::dcast(participant_id + group + model + BIC ~ x, value.var = 'value') %>%
...
# Summarize BIC (and AICc) across groups
...
```{r}
# Plot
p = ggplot(data = Prepare_data_for_plot(data_counts_age),
           aes(x = model,
               y = n_winning,
               fill = model)) +
  geom_col() +
...
data_ep = data_model_comp %>%
  ...
  .[, model := factor(model, levels = data_counts_all$model)] %>%
  .[, ':='(nBIC = -(BIC))] %>%
  data.table::dcast(participant_id + group ~ model, value.var = 'nBIC')
...
```{r}
# Plot
p = ggplot(data = Prepare_data_for_plot(data_pxp),
           aes(x = model,
               y = pxp,
               fill = model)) +
  geom_col() +
  labs(y = 'Protected exceedance probability') +
  scale_fill_manual(values = guides) +
...
data_BIC = data_model_comp %>%
  .[, .(mean_BIC = mean(BIC),
        sd_BIC = sd(BIC),
        sem_BIC = sd(BIC)/sqrt(.N)),
    by = c('model')]

# Display table
table_BIC_all = data_BIC %>%
  .[, c('mean_BIC', 'sd_BIC', 'sem_BIC') := lapply(.SD, round, 2),
    .SDcols = c('mean_BIC', 'sd_BIC', 'sem_BIC')] %>%
  kableExtra::kbl("html", align = 'l') %>%
  kableExtra::kable_classic(full_width = FALSE) %>%
  column_spec(1,
              bold = TRUE) %>%
...
data_plot_mean = Prepare_data_for_plot(data_BIC)
...
p1 = ggplot(data = data_plot,
  ...
  geom_errorbar(data = data_plot_mean,
                inherit.aes = FALSE,
                aes(x = model,
                    ymin = mean_BIC - sem_BIC,
                    ymax = mean_BIC + sem_BIC),
                color = 'black',
                width = 0.3) +
...
# Parametric BIC comparison

Manuscript ll. 416-422

```{r}
# Relative BIC to RW model
data_model_comp_rw = data_model_comp %>%
  data.table::dcast(participant_id + group ~ paste0('BIC_', model),
                    value.var = 'BIC') %>%
  .[, ':='(SmRW = BIC_surprise - BIC_rw,
           SEPLRmRW = BIC_seplr - BIC_rw,
           UmRW = BIC_uncertainty - BIC_rw,
           USEPLRmRW = BIC_uncertainty_seplr - BIC_rw,
           USmRW = BIC_uncertainty_surprise - BIC_rw,
           SSmRW = BIC_seplr_surprise - BIC_rw),
...
data_corr = data %>%
  ...
  data.table::dcast(participant_id + group + BIC ~ x, value.var = 'value') %>%
  data.table::merge.data.table(., check_noc, by = c('participant_id', 'group'))
```
...
data_lrs = data[model == 'surprise' & variable == 'LRs' & !is.na(value)] %>%
  .[, .SD, .SDcols = c('participant_id', 'group', 'model', 'BIC', 'variable',
                       'x', 'value')] %>%
  # Fuse encountered LRs with uml
  data.table::merge.data.table(., data_surprise_param,
                               by = c('participant_id', 'group', 'model', 'BIC'))
...
data_prs = data %>%
  .[iter == 1] %>%
  .[, .(prs = unique(pseudorsq)),
    by = c('participant_id', 'group', 'model')] %>%
  # Highest pseudo-r2 offers best fit compared to chance
  .[, ':='(highest = max(prs),
           loc_winning = prs == max(prs),
           # Name of winning model
           winning_model = model[prs == max(prs)]),
    by = c('participant_id', 'group')]

data_prs_winning = data_prs %>%
  .[, .(count_winning = sum(loc_winning)),
    by = 'model']
```

```{r}
p = ggplot(data = Prepare_data_for_plot(data_prs_winning),
           aes(x = model,
               y = count_winning,
               color = model,
               fill = model)) +
  geom_col() +
  labs(y = 'Count highest Pseudo-R2') +
...
```{r, warning=FALSE}
p = ggplot(data = Prepare_data_for_plot(data_prs),
           aes(x = model,
               y = prs)) +
  geom_boxplot(outlier.shape = NA,
               width = 0.5) +
  stat_summary(geom = 'point', fun = 'mean', shape = 23, color = 'black', fill = 'white', size = 4, stroke = 1.5) +
  geom_line(aes(group = participant_id),
            alpha = 0.1,
            color = 'black') +
  geom_point(aes(color = loc_winning,
                 size = loc_winning),
             alpha = 1) +
  scale_size_manual(values = c(1,3)) +
  labs(y = 'Pseudo-R2 (against chance)') +
  theme(axis.title.x = element_blank(),
        axis.text.x = element_text(angle = 20,
                                   hjust = 1))
...
# BIC-based model recovery (Principled Betas)
...
data_recovery = data %>%
  ...
  .[, .(BIC = unique(BIC)),
    by = c('participant_id',
           ...
           'generating_x5',
           'generating_x6')] %>%
  # Get winning model of recovery (lowest BIC)
  .[, recovered_model := model[which(BIC == min(BIC))],
    by = c('participant_id',
           'generating_model',
           'iter')] %>%
