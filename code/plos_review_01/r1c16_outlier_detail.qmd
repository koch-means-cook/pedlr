...
```{r}
data_perf_est = check_mean_est_diff %>%
...
One outlier did not deliver reliable estimation results (8TYYUVQ).
Bandit effect is stronger above 0 (meaning more errors in low-mid than mid-high) the stronger we underestimate bandit 2.

## Plot estimation bandit 3 vs. bandit effect
...
```{r}
data_est_outlier = data_perf_est %>%
  .[, older_outlier := participant_id %in% outlier_ids$participant_id] %>%
  data.table::melt(id.vars = c('participant_id', 'group', 'older_outlier'),
                   measure.vars = c('est_stim_1', 'est_stim_2', 'est_stim_3')) %>%
...
Performance-based outlier that was not excluded from estimates performs worst in estimates.
...
Idea of reviewer that effect is related to higher alpha_neg is true (more weight of neg events on learning means more mistakes in low-mid trials than mid-high trials).
But same is true for u (more overweighting of extreme outcomes leads to more mistakes in low-mid trials than mid-high trials)
