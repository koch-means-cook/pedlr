...
data_fit_comp = data_fit_comp %>%
  ...
  dcast(participant_id + model + AIC + variable + x ~ paste0('value_', author), value.var = 'value') %>%
...
- Average AIC comparison between sophisticated models and RW:
   - (Y) AIC average over all participants for each model [uncertainty_surprise (242.02) < surprise (242.47) < uncertainty (245.13) < rw (247.40)]
...
data_corr = data %>%
  .[variable == 'coefs' & model == 'rw' & starting_values == 'fixed' & iter == 1] %>%
