...
```{r, out.width='100%'}
data_plot = Prepare_data_for_plot(check_op)
p_oc = ggplot(data = data_plot,
              aes(x = run,
                  y = perc_correct,
                  color = group,
                  fill = group)) +
...
check_ravg = data %>%
  ...
  .[, ':='(avg_1_running = Get_running_avg(choice_option = option_choice,
           ...
           avg_3_running = Get_running_avg(choice_option = option_choice,
                                     ...
                                     stim = 3)),
    by = c('participant_id', 'run')] %>%
  .[, ':='(ravg_2m1 = avg_2_running - avg_1_running,
           ravg_3m2 = avg_3_running - avg_2_running)] %>%
  .[comp == '1v2', ravg_3m2 := NA] %>%
  .[comp == '2v3', ravg_2m1 := NA] %>%
  .[comp == '1v3', ravg_2m1 := NA] %>%
  .[comp == '1v3', ravg_3m2 := NA]
```
...
p_rtc_median = ggplot(data = data_plot,
  ...
  scale_color_manual(values = custom_guides) +
  scale_fill_manual(values = custom_guides) +
  scale_linetype_manual(values = custom_guides) +
