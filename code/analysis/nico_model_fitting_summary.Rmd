...
data = fit_data %>%
  ...
  .[, ':='(model = unlist(strsplit(as.character(variable), split = '_'))[1],
    ...
    by = c('participant_id', 'run', 'fit', 'variable')] %>%
  .[, variable := NULL] %>%
  data.table::dcast(.,
                    participant_id + run + fit + random_x0 + Choice1v2 + Choice2v3 + Rating2 + RatingSD2 + model ~ var, value.var = 'value')
...
for(i in seq(length(unique(data$fit)))){
  for(j in seq(2)){
    ...
    x = data[model == 'PEDLR' & run == j & fit == i]$value2
    y = data[model == 'PEDLR' & run == j & fit == i]$alpha1
    a1_rating2 = cor.test(x, y)
    
    # do subjs with more evidence for the PEDLR model make more mistakes in bandit 2?
    x = data[model == 'RW' & run == j & fit == i]$LL - data[model == 'PEDLR' & run == j & fit == i]$LL
    y = data[model == 'PEDLR' & run == j & fit == i]$Choice1v2
    diffLL_1v2 = cor.test(x, y)
    x = data[model == 'RW' & run == j & fit == i]$LL - data[model == 'PEDLR' & run == j & fit == i]$LL
    y = data[model == 'PEDLR' & run == j & fit == i]$Choice1v2 - data[model == 'PEDLR' & run == j & fit == i]$Choice2v3
    ...
    # a0 vs a1
    x = data[model == 'PEDLR' & run == j & fit == i]$alpha0
    y = data[model == 'PEDLR' & run == j & fit == i]$alpha1
    a0_a1 = cor.test(x, y)
    
    temp = data.table(fit = i, run = j,
