...
- RW model: People behave like RL models

```{r}
# Get statistics of beta 1 and 2
data_rw_beta = data[model == 'rw' & variable == 'coefs'] %>%
...
# Left bandit
# Younger
t.test(data_rw_beta[group == 'younger']$V2,
       mu = 0)
# Older
t.test(data_rw_beta[group == 'older']$V2,
       mu = 0)

# Right bandit
# Younger
t.test(data_rw_beta[group == 'younger']$V1,
       mu = 0)
# Older
t.test(data_rw_beta[group == 'older']$V1,
       mu = 0)
```
...
# Left bandit
cor.test(data_rw_beta$V2,
         data_rw_beta$perc_correct_1v2)

# Right bandit
cor.test(data_rw_beta$V1,
         data_rw_beta$perc_correct_1v2)
```
...
m1 = lm(cbe ~ param_uml * group,
        data = data_cbe_fit)
summary(m1)
# m2 = lm(cbe ~ param_uml * AICc * group,
#         data = data_cbe_fit)
# summary(m2)
# anova(m1, m2)
...
# Including model fit model over/underweighting (continuous) and group
m1 = lm(cbe ~ param_uml * AICc * group,
        data = data_cbe_fit)
Anova(m1)
...
<!-- #sim_lrfunc = as.data.table(seq(0, 2.5, length.out = 5)) -->
...
<!-- data_uml = data_surprise[variable == 'param_uml'] %>% -->
<!--   .[, uml := value] %>% -->
<!--   .[, uml_pos := uml >= 0] %>% -->
