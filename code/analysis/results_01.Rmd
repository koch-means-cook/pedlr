...
## Robust regression approach (bandit effect) {.tabset}
...
```{r}
# Standard OLS
ols = lm(perc_choice_2 ~ group * run,
         data = check_risk)
summary(ols)
```
...
cooks_d <- cooks.distance(ols)
r <- stdres(ols)
a <- cbind(check_risk, cooks_d, r)
# Conventional cut-off (https://stats.oarc.ucla.edu/r/dae/robust-regression/)
cutoff = 4/nrow(check_risk)
# Only absolute residuals matter
a = a %>%
  .[, ':='(abs_r = abs(r),
           cutoff = cooks_d > cutoff)] %>%
...
```{r}
# Robust Regression model (huber weights)
rr.huber = MASS::rlm(perc_choice_2 ~ group * run,
                     data = check_risk)
bla = summary(rr.huber)
bla
...
```{r}
# Robust regression (bisquare weights)
rr.bisquare = MASS::rlm(perc_choice_2 ~ group * run,
                     data = check_risk,
                     psi = psi.bisquare)
bla = summary(rr.bisquare)
bla
