...
data$model = factor(data$model, levels = c('Rw', 'Pedlr_simple', 'Pedlr_simple_const', 'Pedlr', 'Pedlr_step', 'Pedlr_fixdep', 'Pedlr_interdep'))
...
data_pedlr_family = data_only_best_fit %>%
  .[model != 'Rw',] %>%
  data.table::melt(id.vars = c('participant_id', 'group', 'run', 'real_iter', 'second_ll', 'model', 'best_fitting_model', 'AIC'),
...
data_model = data_best_fit %>%
  ...
  data.table::melt(id.vars = c('participant_id', 'group', 'run', 'real_iter', 'second_ll', 'model', 'best_fitting_model', 'AIC'),
                   measure.vars = c('alpha1', 'temperature'),
                   variable.name = 'para') %>%
  data.table::dcast(participant_id + group + run + real_iter + second_ll + model + best_fitting_model + AIC ~ para, value.var = 'value')
...
for(i_id in unique(data_model$participant_id)){
  for(i_run in unique(data_model$run)){
...
```{r}
bla = lm(mean_diff_from_true_2 ~ alpha1,
...
for(i_id in unique(data_rat$participant_id)){
  for(i_run in unique(data_rat[participant_id == i_id]$run)){
    
    # Select model parameters
    data_fit = data_rat[real_iter == 1 & participant_id == i_id & run == i_run]
...
```{r}
# Rw vs. Pedlr_simple
t.test(data_sse[model == 'Rw' & option == '2', mean_rmse],
       data_sse[model == 'Pedlr_simple' & option == '2', mean_rmse],
       paired = TRUE)

# Rw vs. Pedlr_simple_const
t.test(data_sse[model == 'Rw' & option == '2', mean_rmse],
       data_sse[model == 'Pedlr_simple_const' & option == '2', mean_rmse],
       paired = TRUE)

# Rw vs Pedlr
t.test(data_sse[model == 'Rw' & option == '2', mean_rmse],
       data_sse[model == 'Pedlr' & option == '2', mean_rmse],
       paired = TRUE)

# Rw vs Pedlr_step
t.test(data_sse[model == 'Rw' & option == '2', mean_rmse],
       data_sse[model == 'Pedlr_step' & option == '2', mean_rmse],
       paired = TRUE)

# Rw vs. Pedlr_fixdep
t.test(data_sse[model == 'Rw' & option == '2', mean_rmse],
       data_sse[model == 'Pedlr_fixdep' & option == '2', mean_rmse],
       paired = TRUE)

# Rw vs. Pedlr_interdep
t.test(data_sse[model == 'Rw' & option == '2', mean_rmse],
       data_sse[model == 'Pedlr_interdep' & option == '2', mean_rmse],
       paired = TRUE)

lme = lme4::lmer(mean_rmse ~ group * model * option + (1 | participant_id),
                 data = data_sse)
Anova(lme)
```
...
```{r}
lme = lme4::lmer(preminuspost ~ group * weight_high_pe + run + (1 | participant_id),
...
```{r}
lme = lme4::lmer(mean_diff_from_true_2 ~ group * weight_high_pe + run + (1 | participant_id),
...
data_avg_update = data_pe_comp %>%
  ...
  # Get trialwise absolute update 
  .[, abs_update := abs(value) * alpha1,
    by = c('participant_id', 'group', 'run', 'trial')] %>%
  # Mean asolute update in case of rare PE
  .[, .(mean_abs_update = mean(abs_update),
        sd_abs_update = sd(abs_update)),
    by = c('participant_id', 'group', 'run', 'model', 'best_fitting_model')]
```
