...
Sample-wide AIC comparison (Pedlr_step vs. Rw): Pedlr_step wins
...
data_perm = do.call("rbind", replicate(n_perm, data_sanity_split, simplify = FALSE)) %>%
  ...
  .[, weight_high_pe := sample(weight_high_pe),
    by = 'perm_iter'] %>%
...
ggplot(data = data_perm,
       aes(x = weight_age_diff)) +
  geom_histogram(binwidth = 2) +
  geom_vline(xintercept = real_weight_age_diff,
             color = 'red')

# Get percentage of samples lower than real value
mean(data_perm$weight_age_diff < real_weight_age_diff)

quantile(data_perm$weight_age_diff, probs = .025)
```
...
```{r}
lm = lm(preminuspost ~ AIC * weight_high_pe * group,
   data = data_sanity_split)
Anova(lm)

lm = lm(preminuspost ~ AIC * weight_high_pe,
   data = data_sanity_split)
Anova(lm)
```
