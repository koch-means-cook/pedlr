...
```{r}
data_pedlr_family = data_best_fit %>%
  .[model != 'Rw',] %>%
...
```{r}
data_model = data_pedlr_family %>%
  data.table::dcast(participant_id + group + run + real_iter + second_ll + model + AIC ~ para, value.var = 'value')
...
```{r}
data_alpha_diff = data_best_fit %>%
  .[model == 'Pedlr_step',] %>%
  .[, diff_1m0 := alpha1 - alpha0]

ggplot(data = data_alpha_diff,
       aes(x = group,
           y = diff_1m0,
           color = group,
           fill = group)) +
...
```{r}
t.test(data_alpha_diff[group == 'older']$diff_1m0,
       data_alpha_diff[group == 'younger']$diff_1m0,)
```
