...
```{r}
p = ggplot(data = Prepare_data_for_plot(data_sanity_rat),
       aes(x = alpha1,
...
```{r}
t.test(data_alpha_ratio[group == 'younger']$alpha0,
...
# Alignment rare outcomes and PEs flagged as rare by RW model

```{r}
data_flag_check = data_est_values %>%
  Add_comp(.) %>%
  .[, trial := seq(.N),
    by = c('participant_id', 'group', 'run')] %>%
  # Get PE for each trial, regardless of choice
  .[, Pedlrstep_overall_pe := sum(c(Pedlrstep_1_pe, Pedlrstep_2_pe, Pedlrstep_3_pe),
                                  na.rm = TRUE),
    by = c('participant_id', 'group', 'run', 'trial')] %>%
  # See which PEs get flagged as surprising outcome
  .[, rare_flag_rw := abs(Pedlrstep_overall_pe) > pe_95th_Rw,
    by = c('participant_id', 'group', 'run', 'trial')] %>%
  # Get actual rare outcomes collected by participant
  .[, actual_rare_outcome := is_rare == TRUE & option_choice == 2,
    by = c('participant_id', 'group', 'run', 'trial')] %>%
  # Narrow down columns
  .[, c('participant_id', 'group', 'run', 'trial', 'trial_type', 'is_rare',
        'option_choice', 'comp', 'pe_95th_Rw', 'Pedlrstep_overall_pe',
        'rare_flag_rw', 'actual_rare_outcome')]

  # ROC
data_flag_roc = data_flag_check %>%
  .[, ':='(false_positive = (rare_flag_rw == TRUE & actual_rare_outcome == FALSE),
           false_negative = (rare_flag_rw == FALSE & actual_rare_outcome == TRUE),
           correct_rejection = (rare_flag_rw == FALSE & actual_rare_outcome == FALSE),
           correct = (rare_flag_rw == TRUE & actual_rare_outcome == TRUE))] %>%
  .[, .(false_positive = sum(false_positive, na.rm = TRUE) / .N,
        false_negative = sum(false_negative, na.rm = TRUE) / .N,
        correct_rejection = sum(correct_rejection, na.rm = TRUE) / .N,
        correct = sum(correct, na.rm = TRUE) / .N),
    by = c('group')] %>%
  data.table::melt.data.table(measure.vars = c('false_positive', 'false_negative', 'correct_rejection', 'correct'))
```

```{r}
# RW flag ROC
data_plot = Prepare_data_for_plot(data_flag_roc)
ggplot(data = data_plot[variable != 'correct_rejection'],
       aes(x = variable,
           y = value,
           group = group,
           color = group)) +
...
Out of all real rare outcomes: How many do we catch

```{r}
data_flag_check[is_rare == 1 & option_choice == 2] %>%
  .[, .(perc_captured_rare = mean(rare_flag_rw, na.rm = TRUE)),
    by = c('group')]
```


```{r}
data_flag_check[rare_flag_rw == TRUE & trial > 10]
```


```{r}
ggplot(data_pe_dist[trial > 10],
       aes(x = value)) +
  geom_histogram()
```
