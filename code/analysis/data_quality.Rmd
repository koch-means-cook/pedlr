...
### Less than 55% overall accuracy

```{r}
excl = sus_pool[reason == 'Less than 55% overall accuracy']
...
```{r}
excl = rem[reason == 'Consistently similar estimate for bandit 1 and 3']
ids = excl$participant_id
rem = rem[!participant_id %in% ids, ]
excl
```

### Less than 55% accuracy chosing between bandit 1 and 3

```{r}
excl = rem[reason == 'Less than 55% accuracy chosing between bandit 1 and 3']
ids = excl$participant_id
rem = rem[!participant_id %in% ids, ]
excl
```

### Less than 75% accuracy in forced choices

```{r}
excl = rem[reason == 'Less than 75% accuracy in forced choices']
ids = excl$participant_id
rem = rem[!participant_id %in% ids, ]
excl
```
