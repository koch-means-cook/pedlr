...
Get_best_fit = function(data){
  result = data.table::setDT(data) %>%
    ...
    .[, fit := seq(.N),
      by = c('participant_id', 'run', 'model','para')] %>%
    # Add number of parameters
    .[, n_para := length(para),
      by = c('participant_id', 'run', 'model', 'fit')] %>%
    # Cast into long format (one line per fit attempt)
    data.table::dcast(., fit + participant_id + run + name_design_r1 + name_design_r2 + model + n_para + second_ll ~ para,
